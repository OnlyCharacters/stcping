...
int
tcping4_epoll(struct sockaddr* servaddr)
{
    int                 n;
    int                 error;
    int                 sockfd;
    ...
    if (epoll_ctl(epollfd, EPOLL_CTL_ADD, sockfd, &ev) == -1) {
        perror("tcping4.epoll_ctl() add sockfd");
        return 1;
    }
    ...
    // not yet connect
        ...
        // timeout
        if ((n = epoll_wait(epollfd, events, MAX_EVENTS, 2000)) == 0) {
            close(sockfd);
            if (gettimeofday(&after, NULL)) {
                perror("tcping4.gettimeofday(&after, NULL)");
                fprintf(stderr, "Timeout %ld ms\n", THOUSAND * timeout.tv_sec);
        ...
        // error
            ...
            close(sockfd);
            perror("tcping4.epoll_wait()");
            ping_times++;
            return 1;
        }
        // connect or connect error
        else {
            // get time first
            if (gettimeofday(&after, NULL)) {
                close(sockfd);
...
int
tcping6_epoll(struct sockaddr* servaddr)
{
    int                 n;
    int                 error;
    int                 sockfd;
    ...
    ev.data.fd = sockfd;
    if (epoll_ctl(epollfd, EPOLL_CTL_ADD, sockfd, &ev) == -1) {
        perror("tcping6.epoll_ctl() add sockfd");
        return 1;
    }
...
prepare_tcping(struct addrinfo* item, int port)
{
    ...
    if (item->ai_family == AF_INET) {
        ...
        if (stop_times == -1) {
            for ( ; ; ) {
                tcping4_epoll((struct sockaddr*)&sinaddr);
                sleep(2);
            }
        }
        else {
        ...
        if (stop_times == -1) {
            for ( ; ; ) {
                tcping6_epoll((struct sockaddr*)&sinaddr6);
                sleep(2);
            }
        }
        else {
